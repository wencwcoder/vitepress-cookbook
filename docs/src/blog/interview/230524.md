...
4. 类型检测
   ...
   - 每个对象都有一个 **constructor** 属性，指向创建该对象的**构造函数**。
5. 类型转换
...
1. 相关概念
   ...
   - JavaScript 中**自由变量的查找是在函数定义的地方**，向上级作用域查找，不是在执行的地方。 
...
3. 作用域扩展
   ...
   2. **作用域链**：当查找一个变量时，JavaScript 引擎会首先在当前作用域中查找，如果未找到，它会沿着作用域链向上查找，直到全局作用域。如果还未找到，则返回 `undefined`。**作用域决定了代码中变量和其他资源的可见性，而作用域链则是变量查找的路径**。
...
- 【4】根据构造函数返回类型作判断，如果返回值是一个对象，则返回该对象；否则返回新创建的对象（需要特别注意一下，上面有对比例子）。
...
- **事件冒泡（Event Bubbling）**：事件先从目标元素开始，一层一层向上传播到根元素。
- **事件捕获（Event Capturing）**：事件从根元素一层一层向下传播到目标元素。
- **事件委托（Event Delegation）**：事件冒泡机制允许我们使用事件委托（Event Delegation）技术，以减少事件监听器的数量，从而提高性能。例如，把所有子元素的点击事件委托给父元素处理：
...
3. async/await
   ...
   `async` 和 `await` 是**基于 Promise 的语法糖**，使得异步代码看起来更像同步代码，增强了可读性和可维护性。`async` 函数返回一个 Promise，而 `await` 表达式用于等待 Promise 完成，并返回 Promise 的结果。
...
2. 特性
   ...
   - **不绑定自己的`this`**：箭头函数不绑定自己的`this`，它会**捕获其所在上下文的`this`值，作为自己的`this`值**，这使得在回调函数中处理`this`变得更加容易：
...
### 2.12 CommonJS 和 ES Module 的区别

CommonJS 和 ES Module 是 JavaScript 中两种不同的**模块化规范**，它们在定义、使用以及特性上存在显著的差异。以下是对这两者的详细比较：
...
### 2.13 setTimeout 和 setInterval 的区别
...
### 2.14 setTimeout 不适合做动画

1. **无法与屏幕刷新同步**：它的执行时机不保证在重绘之前，容易导致丢帧或过度绘制。
2. **无自动节流与后台暂停**：页面不可见时仍会执行，浪费资源；而 `requestAnimationFrame` 会在后台暂停。
3. **精度和稳定性差**：固定间隔无法适应不同刷新率的屏幕，且受事件循环阻塞影响，动画不平滑。

因此，现代动画应该使用 `requestAnimationFrame` 或 CSS 动画/过渡。



### 2.15 for...in 和 for...of 的区别
...
4. 数值转换
   - undefined 转换为数值时返回 NaN。
   - null 转换为数值时返回 0。



### 2.17 如何获取安全的 undefined 值
...
### 2.19 Object.assign 和对象扩展运算符的区别

1. 对目标对象的影响：
   - `Object.assign`：**修改第一个参数对象（目标对象），然后将该对象返回**。
     例如：`const target = { a: 1 }; Object.assign(target, { b: 2 });` → `target` 被直接修改。
   - 扩展运算符：**创建一个全新的对象**，不会修改任何已有对象。
     例如：`const target = { a: 1 }; const newObj = { ...target, b: 2 };` → `target` 保持不变。
2. 是否触发 setter
   - `Object.assign`：在赋值过程中会触发目标对象上的 **setter** 方法（如果存在）。
   - 扩展运算符：在创建新对象时，只是简单地定义属性，**不会触发任何 setter**。
3. 处理 `null` / `undefined` 源对象
   - `Object.assign`：如果第一个参数是 `null` 或 `undefined`，会报错（因为它要求目标必须是一个对象）。
     但源对象为 `null` / `undefined` 时会被**忽略**。
   - 扩展运算符：`{ ...null }` 或 `{ ...undefined }` 会得到**空对象** `{}`，不会报错。



### 2.20 encodeURI、encodeURIComponent
...
2. **尾递归（Tail Recursion）**：
   ...
   function factorialTail(n, acc = 1) {
     if (n === 1) return acc;
     return factorialTail(n - 1, n * acc); // 是尾调用，可以复用调用帧
   }
   ```
...
1. 算术运算
   ...
   注意：**如果两个操作数中有一个是字符串，则 + 操作符将执行字符串拼接**。
...
2. 使用示例
   ...
   - 使用 BigInt 可以非常简单，只需**在整数后加上 n 后缀**，或者使用 BigInt 构造函数，示例如下：
...
### 3.18 如何提取深度嵌套的对象中的指定属性

从深度嵌套的对象中提取指定属性，关键是**安全地访问可能不存在的中间路径**，避免因 `undefined` 或 `null` 导致报错。以下是几种常用方法：

1. 可选链操作符（`?.`） — 最推荐（ES2020+）

   直接链式访问，遇到 `null` 或 `undefined` 会短路返回 `undefined`，不会报错。

   ```javascript
   const value = obj?.a?.b?.c?.target; // 如果中间任何一层缺失，value === undefined
   ```

2. 解构赋值 + 默认值（仅当层级固定且较少时）

   ```javascript
   const { a: { b: { c: { target = '默认值' } = {} } = {} } = {} } = obj;
   ```

3. Lodash 的 `_.get` 方法

   ```javascript
   const target = _.get(obj, 'a.b.c.target', '默认值');
   ```
