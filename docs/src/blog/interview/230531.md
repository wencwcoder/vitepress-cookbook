...
2. 特殊类型
   ...
   - 3）never：表示**不会发生的值**，通常用于标识函数从不会返回（如抛出异常、进入无限循环）或永远不会有结果的情况。例如：
...
3. 访问修饰符
   ...
   - `public`（**默认**）：成员可以在**任何地方**被访问，没有限制。
   - `private`：成员只能在声明它的**类内部**被访问，不能在类的实例以及子类中访问。
   - `protected`：成员只能在声明它的**类及其子类**中被访问，不能在类的实例中访问。
...
条件类型是 TypeScript 中的一种高级类型工具，它允许我们**根据一个条件来选择类型**。条件类型的语法类似于 JavaScript 的三元运算符，用来根据某个条件来选择一个类型。其基本的形式是 `T extends U ? X : Y`，意思是如果 **T 能赋值给 U**，那么类型结果为 X，否则为 Y。
...
TypeScript 的映射类型是一种高级类型，它允许我们**根据已有类型生成新的类型**。通过使用映射类型，可以将特定的变换应用到类型的属性上，生成一个新的类型。例如，假设我们有一个接口 `Person`，其中包含一些属性，现在我们想要创建一个新类型，将 `Person` 中所有属性都设为可选属性（`Partial`类型）；或者我们想要创建一个新类型，使 `Person` 中所有属性变为只读（`Readonly`类型）。这些操作都可以通过映射类型来实现。
...
**逆变（Contravariance）** 和**协变（Covariance）** 是类型系统中关于**子类型关系如何随复合类型变化**的规则。简单说：当你用 `A` 和 `B` 构造出更复杂的类型 `T<A>` 和 `T<B>` 时，`T<A>` 与 `T<B>` 之间的子类型关系是怎样的？

1. 基本定义

   - **协变**：保持方向。

     若 `A` 是 `B` 的子类型，则 `T<A>` 也是 `T<B>` 的子类型。→ 即 `T<子类型>` 是 `T<父类型>` 的子类型。

   - **逆变**：反转方向。

     若 `A` 是 `B` 的子类型，则 `T<B>` 是 `T<A>` 的子类型（或说 `T<A>` 是 `T<B>` 的超类型）。→ 即 `T<父类型>` 是 `T<子类型>` 的子类型。

   - **不变（Invariant）**：既不协变也不逆变。

     即使 `A` 是 `B` 的子类型，`T<A>` 与 `T<B>` 之间也没有子类型关系。

2. 为什么需要区分？

   - **安全性**：保障类型系统符合里氏替换原则（LSP）。例如，数组协变在 TypeScript 中是不安全的（允许 `animals.push(new Cat())` 通过类型检查但运行时出错），所以需要 `readonly` 数组或 `--strictFunctionTypes` 标志来收紧。
   - **泛型设计**：当设计泛型类或接口时，明确标注协变（`out T`）或逆变（`in T`）可以让编译器检查并让使用者清晰知道能做什么。
     - Kotlin：`out T`（协变，生产者），`in T`（逆变，消费者）
     - TypeScript：通过函数类型的双向检查历史上有些例外，但使用 `--strict` 模式下参数位置默认是逆变的。



## 3. TypeScript 拓展

### 3.1 使用 TypeScript 进行开发时，有什么心得体会
...
TypeScript 中的关键字 `infer` 通常与条件类型（conditional types）一起使用，用于在类型检查中过度复杂或者未知的类型结构中进行**类型推断**。它可以**从类型中“提取”类型变量**，使得我们可以在条件类型的 `extends` 分支中对其进行操作。这是在处理复杂类型转换和类型推断时非常强大且有用的工具。
...
以下工具类型均在 `@typescript` 核心库中，无需额外安装。

### 6.1 属性修改（映射类型）

| 工具类型       | 作用                                 | 示例                                                         |
| :------------- | :----------------------------------- | :----------------------------------------------------------- |
| `Partial<T>`   | 将 `T` 的所有属性变为可选            | `Partial<{ x: number; y: number }>` → `{ x?: number; y?: number }` |
| `Required<T>`  | 将 `T` 的所有属性变为必选            | `Required<{ x?: number }>` → `{ x: number }`                 |
| `Readonly<T>`  | 将 `T` 的所有属性变为只读            | `Readonly<{ x: number }>` → `{ readonly x: number }`         |
| `Pick<T, K>`   | 从 `T` 中选取一组属性 `K`            | Pick<{ a:1; b:2; c:3 },  'a' \| 'c'> → `{ a:1; c:3 }`        |
| `Omit<T, K>`   | 从 `T` 中排除一组属性 `K`            | Omit<{ a:1; b:2; c:3 }, 'a'> → `{ b:2; c:3 }`                |
| `Record<K, T>` | 构造一个对象类型，键为 `K`，值为 `T` | Record<'x' \| 'y', number> → `{ x: number; y: number }`      |
...
| `Extract<T, U>`  | 从 `T` 中提取可赋值给 `U` 的类型             | Extract<'a'\|'b'\|'c', 'a'\|'b'> → 'a'\|'b' |
| `NonNullable<T>` | 从 `T` 中排除 `null` 和 `undefined`          | NonNullable<string\|null> → string          |
| `NoInfer<T>`     | 阻止类型推断（用于泛型参数中提示不进行推理） | `createWrapper<NoInfer<Shape>>(shape)`      |
...
| 工具类型                   | 作用                           | 示例                                                         |
| :------------------------- | :----------------------------- | :----------------------------------------------------------- |
| `Parameters<T>`            | 获取函数类型的参数类型（元组） | `Parameters<(a: string, b: number) => void>` → `[string, number]` |
| `ReturnType<T>`            | 获取函数类型的返回值类型       | `ReturnType<() => string>` → `string`                        |
| `ConstructorParameters<T>` | 获取构造函数类型的参数类型     | `ConstructorParameters<ErrorConstructor>` → `[message?: string]` |
| `InstanceType<T>`          | 获取构造函数类型的实例类型     | `InstanceType<typeof Array>` → `any[]`                       |
| `ThisParameterType<T>`     | 获取函数的 `this` 参数类型     | `ThisParameterType<(this: Date) => void>` → `Date`           |
| `OmitThisParameter<T>`     | 移除函数的 `this` 参数类型     | 返回无 `this` 标注的函数类型                                 |
...
| 工具类型          | 作用                       | 示例                                |
| :---------------- | :------------------------- | :---------------------------------- |
| `Uppercase<S>`    | 将字符串字面量类型转为大写 | `Uppercase<'hello'>` → `'HELLO'`    |
| `Lowercase<S>`    | 将字符串字面量类型转为小写 | `Lowercase<'HELLO'>` → `'hello'`    |
| `Capitalize<S>`   | 将字符串首字母大写         | `Capitalize<'hello'>` → `'Hello'`   |
| `Uncapitalize<S>` | 将字符串首字母小写         | `Uncapitalize<'Hello'>` → `'hello'` |
...
| 工具类型      | 作用                                                         | 示例                                           |
| :------------ | :----------------------------------------------------------- | :--------------------------------------------- |
| `Awaited<T>`  | 递归展开 Promise 类型（获取 `Promise` 内的最终值类型）       | `Awaited<Promise<Promise<string>>>` → `string` |
| `ThisType<T>` | 用于对象字面量中指定 `this` 类型（需要开启 `noImplicitThis`） | 通常在 `defineProperty` 等 API 中使用          |
