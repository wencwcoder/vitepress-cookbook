...
在 React Router 的 history 模式中，push 和 replace 两个方法的主要区别在于它们如何管理浏览**历史记录**。
...
在 React Router 中，获取 URL 参数主要分为**路径参数**（如 `/user/:id`）和**查询参数**（如 `?name=123`）两种情况。

| 参数类型     | React Router v6 (现代写法 - Hooks)                         | React Router v5 (早期写法)          |
| :----------- | :--------------------------------------------------------- | :---------------------------------- |
| **路径参数** | `useParams()`                                              | `props.match.params`                |
| **查询参数** | `useSearchParams()` 或 `useLocation()` + `URLSearchParams` | `useLocation()` + `URLSearchParams` |



### 1.6 Link 标签和 a 标签 的区别

**`<Link>`** 是在 `<a>` 基础上的增强封装，**本质仍然是 `<a>` 标签**，但它**阻止了默认的跳转行为**，并通过 History API 实现了**无刷新**的前端路由。

| 对比维度         | `<a>` 标签                                              | `<Link>` 组件 (React Router)                                 |
| :--------------- | :------------------------------------------------------ | :----------------------------------------------------------- |
| **页面刷新**     | ✅ **会刷新整个页面**，重新加载 HTML、CSS、JS 等所有资源 | ❌ **不会刷新页面**，仅更新需要变化的组件                     |
| **浏览器历史**   | 正常添加历史记录                                        | 同样会添加历史记录（通过 `pushState`）                       |
| **路由切换方式** | 发送 HTTP 请求到服务器 → 服务器返回新页面               | 前端路由通过 `history API` 拦截，不请求服务器                |
| **应用状态**     | ❌ **丢失所有 React 状态** (Redux、组件状态等)           | ✅ **保持应用状态**（只要组件不卸载）                         |
| **渲染性能**     | 慢（重新加载整个应用）                                  | 快（只重新渲染路由匹配的组件）                               |
| **使用场景**     | 跳转到**外部网站**或**非 SPA 页面**                     | SPA **内部路由**导航                                         |
| **额外功能**     | 无                                                      | 支持 `to` 对象、`replace`、`state` 传递、`activeClassName` (NavLink) 等 |
| **默认行为**     | 触发浏览器的默认导航事件                                | 阻止默认事件，调用 `history.push` 或 `replace`               |
| **最终渲染结果** | 本身就是 `<a>`                                          | 最终也会渲染成 `<a>`，但附加了 onClick 处理逻辑              |
