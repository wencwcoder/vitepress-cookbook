...
1. **原型（Prototype）**：
   ...
   每个 JavaScript 对象都有一个**内部属性**叫做 `[[Prototype]]`，这通常可以通过 `__proto__` 来访问。这个 `[[Prototype]]` 指向另一个对象，从而形成了所谓的原型链。

2. **原型链**：

   JavaScript 的**原型链是一个机制，通过它可以实现对象间的属性继承**。当访问一个对象的属性时，JavaScript 引擎首先会看这个属性是否存在于对象自身。如果不存在，那么它会沿着这个对象的原型链向上查找，直到找到该属性或到达原型链的顶端（通常是 `Object.prototype`），在这种情况下返回 `undefined`。
...
8. **如何获得对象非原型链上的属性**：
   ...
   要获取对象**自身**的属性（即非原型链上的属性），核心思想是**排除继承来的属性**。

   - `Object.keys(obj)`
   - `Object.getOwnPropertyNames(obj)`
   - `Object.getOwnPropertySymbols(obj)`
   - `Reflect.ownKeys(obj)`
   - 手动检查：`Object.hasOwn(obj, prop)` 或 `obj.hasOwnProperty(prop)`：用于判断**单个属性**是否为自身属性（非继承）。推荐使用 `Object.hasOwn()`（ES2022），更安全（避免对象原型被覆盖）。
